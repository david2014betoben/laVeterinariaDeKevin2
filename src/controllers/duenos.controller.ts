...
import { duenoSchema, actualizarDuenoSchema } from "../schemas/duenos.schema.js";
...
duenosRouter.get("/", async (req, res, next) => {
  try {
    ...
    const duenos = await obtenerDueno();
    res.json(duenos);
  } catch (err) {
    next(err);
  }
...
duenosRouter.put("/:id", validate(actualizarDuenoSchema), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
